    ...
    <div className="container mx-auto p-8 bg-gray-50 min-h-screen">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        ...
        <div className="bg-white rounded-xl shadow-sm p-6 transition-all hover:shadow-md">
          ...
          <p className="text-2xl font-bold text-emerald-600">{dashboard?.totalRevenue}</p>
        </div>
        <div className="bg-white rounded-xl shadow-sm p-6 transition-all hover:shadow-md">
            ...
            {dashboard?.recentOrders.map((item) => (
              <div key={item.id} className="border-b border-gray-100 pb-4">
                <div className="flex justify-between items-center mb-2">
                  <span className="text-sm text-gray-600">Buyurtma #{item.id}</span>
                  <span className={`px-3 py-1 rounded-full text-xs font-medium ${
                    item.status === "Yetkazildi" ? "bg-green-100 text-green-700" :
                    item.status === "Jarayonda" ? "bg-blue-100 text-blue-700" :
                ...
                <div className="flex justify-between text-sm">
                  <span className="text-gray-600">Mijoz ID: {item.customerId}</span>
            ...
            {dashboard?.topProducts.map((item) => (
              <div key={item.id} className="flex items-center justify-between">
                <div className="flex-1">
                  ...
                  <p className="text-sm text-gray-500 mt-1">{item.totalSold} ta sotilgan</p>
                </div>
